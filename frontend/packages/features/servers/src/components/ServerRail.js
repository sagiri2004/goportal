                        ...
                        servers.map((server) => {
                            ...
                            return (_jsxs("div", { className: "relative w-12 group", onMouseEnter: () => setHoveredServerId(server.id), onMouseLeave: () => setHoveredServerId(null), children: [_jsx("div", { className: [
                                            ...
                                            isActive ? 'h-8' : 'h-2 group-hover:h-5',
                                                        ...
                                                        : 'rounded-[24px] bg-secondary text-foreground hover:bg-indigo-500 hover:text-white hover:rounded-[16px]'}`, type: "button", children: server.initials ?? getServerInitials(server.name) }) }), _jsx(TooltipContent, { children: server.name })] })] }, server.id));
