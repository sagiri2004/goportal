...
% add thêm 
\usepackage{xcolor}       % màu header cam (có thể đã có, kiểm tra lại)
\usepackage{colortbl}     % tô màu ô bảng (\cellcolor)
\usepackage{makecell}     % xuống dòng trong ô bảng
\usepackage{ragged2e}     % căn lề RaggedRight trong ô
\usepackage{outlines}     % hỗ trợ môi trường outline trong phụ lục

\definecolor{hdr}{RGB}{204,119,34}
...
\def \TITLE{THIẾT KẾ VÀ XÂY DỰNG NỀN TẢNG CỘNG ĐỒNG GAME TÍCH HỢP GIAO TIẾP THỜI GIAN THỰC VÀ QUẢN LÝ GIẢI ĐẤU}
\def \AUTHOR{NGUYỄN HỮU THẮNG}
...
\titlecontents{chapter}
    ...
    {}         % unnumbered format
    {\titlerule*[0.3pc]{.}\contentspage}         % filler-page-format, e.g dots
\chapter{ĐẶC TẢ USE CASE TIÊU BIỂU}
