...
var registry = map[string]errDef{
	...
	"TOURNAMENT_NOT_FOUND":                 {http.StatusNotFound, "Tournament not found"},
	"TOURNAMENT_FORBIDDEN":                 {http.StatusForbidden, "You are not allowed to manage this tournament"},
	"TOURNAMENT_INVALID_FORMAT":            {http.StatusBadRequest, "Invalid tournament format"},
	"TOURNAMENT_INVALID_PARTICIPANT_TYPE":  {http.StatusBadRequest, "Invalid tournament participant type"},
	"TOURNAMENT_INVALID_TEAM_SIZE":         {http.StatusBadRequest, "Invalid tournament team size"},
	"TOURNAMENT_REGISTRATION_CLOSED":       {http.StatusBadRequest, "Tournament registration is closed"},
	"TOURNAMENT_CHECKIN_CLOSED":            {http.StatusBadRequest, "Tournament check-in is closed"},
	"TOURNAMENT_PARTICIPANT_EXISTS":        {http.StatusConflict, "Participant is already registered"},
	"TOURNAMENT_PARTICIPANT_NOT_FOUND":     {http.StatusNotFound, "Tournament participant not found"},
	"TOURNAMENT_MAX_PARTICIPANTS_REACHED":  {http.StatusBadRequest, "Tournament maximum participants reached"},
	"TOURNAMENT_TEAM_NOT_FOUND":            {http.StatusNotFound, "Tournament team not found"},
	"TOURNAMENT_TEAM_SIZE_EXCEEDED":        {http.StatusBadRequest, "Team size exceeded"},
	"TOURNAMENT_UPDATE_FORBIDDEN":          {http.StatusBadRequest, "Tournament cannot be updated in current status"},
	"TOURNAMENT_DELETE_FORBIDDEN":          {http.StatusBadRequest, "Tournament can only be deleted in draft status"},
	...
	"GAME_EVENT_TYPE_INVALID":              {http.StatusBadRequest, "Game event type is invalid"},
	"GAME_SESSION_EXPIRED":                 {http.StatusBadRequest, "Game session is expired"},
	"DUPLICATE_IDEMPOTENCY_KEY":            {http.StatusConflict, "Duplicate idempotency key"},
